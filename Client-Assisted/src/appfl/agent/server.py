...
class ServerAgent:
    ...
    def global_update(
        ...
        **kwargs,
        ...
        #TODO deserializing timing
        start_deserializing = time.perf_counter()

        #TODO deserialize parameters
        if 'ckks_context' in kwargs:
            # TODO :D Deserialize the parameters
            self.ckks_context = ts.context_from(kwargs['ckks_context'])
        for name, serialized_param in local_model.items():
            local_model[name] = ts.ckks_vector_from(self.ckks_context, serialized_param)

        deserializing_time = time.perf_counter() - start_deserializing

        if self.training_finished():
            self.logger.info("Training is finished and returning global model.")
            ...
            #TODO timing
            start_aggregating_serialzing = time.perf_counter()

            global_model = self.scheduler.schedule(client_id, local_model, **kwargs)

            aggregating_serializing_time = time.perf_counter() - start_aggregating_serialzing

            #TODO logging
            with open(f'server_timing_log.txt', 'a') as f:
                f.write(
                    f"Deserialize time: {deserializing_time:.4f}s, Aggregation+Serialization time: {aggregating_serializing_time:.4f}s \n")
