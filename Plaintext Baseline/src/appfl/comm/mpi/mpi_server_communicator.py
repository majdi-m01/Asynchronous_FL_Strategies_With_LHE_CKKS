...
class MPIServerCommunicator:
    ...
    def _update_global_model(
        ...
        if meta_data.get("_torch_serialized", True):
            # TODO timing
            start_deserialzing = time.perf_counter()

            local_model = byte_to_model(local_model)

            # TODO end timing and log
            deserializing_time = time.perf_counter() - start_deserialzing

            # TODO logging
            with open(f'server_timing_log.txt', 'a') as f:
                f.write(
                    f"Deserialize time: {deserializing_time:.4f}s, ")
        ...
        for client_id in client_ids:
            ...
            if not isinstance(global_model, Future):
                ...
                with open(f'server_timing_log.txt', 'a') as f:
                    f.write(
                        f"Aggregation+Serialization time: {aggregating_serializing_time:.4f}s \n")
