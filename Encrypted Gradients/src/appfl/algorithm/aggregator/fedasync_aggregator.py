...
class FedAsyncAggregator(BaseAggregator):
    ...
    def __init__(
        ...
        self.staleness_fn = self.__staleness_fn_factory(
            staleness_fn_name=self.aggregator_configs.get("staleness_fn", "polynomial"),
        ...
        # TODO In global_step=0, it is actually a plaintext of random parameters the server generated, not the clients!
    ...
    def aggregate(
        ...
        **kwargs,
        ...
        for name in self.encryptedModelWeights:
            self.encryptedModelWeights[name] += alpha_t * (-local_model[name])
