...
from appfl.algorithm.trainer.LSUV import lsuv_init
...
class VanillaTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Initialize LSUV if configured
        if hasattr(self.train_configs, "init_configs") and self.train_configs.init_configs.get("use_lsuv", False):
            init_configs = self.train_configs.init_configs

            self.model = lsuv_init(
                model=self.model,
                data_loader=self.train_dataloader,
                needed_std=init_configs.get("needed_std", 1.0),
                std_tol=init_configs.get("std_tol", 0.1),
                max_attempts=init_configs.get("max_attempts", 10),
                do_orthonorm=init_configs.get("do_orthonorm", True),
    ...
    def train(self, **kwargs):
        ...
        if do_pre_validation:
            ...
            with open("metrics_log.txt", 'a+') as file:
                # Move to the 10th line
                file.seek(0, 2)  # Seek to the end of the file
                current_pos = file.tell()
                # Check how many lines are in the file
                file.seek(0, 0)
                lines = file.readlines()
                # Starting from the 11th line, write the new data
                line_number = len(lines) - 10 + 1
                file.write(f"{line_number}. Loss: {val_loss}, Accuracy: {val_accuracy}\n")
