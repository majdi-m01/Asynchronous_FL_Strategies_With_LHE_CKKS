...
from .serializer import request_to_byte, byte_to_response, byte_to_model, model_to_byte, response_to_byte
...
class MPIClientCommunicator:
    ...
    def update_global_model(
        ...
        **kwargs,
        ...
        # TODO Log communication overhead client -> server
        with open(f'client_{self.comm.Get_rank()}_comm_log.txt', 'a') as f:
            f.write(f"client_{self.comm.Get_rank()} SENDING an MPITaskRequest of size {len(request_bytes)} bytes \n")

        self.comm.Send(request_bytes, dest=self.server_rank, tag=tag)
        response = self._recv_response()

        # TODO Log communication overhead server -> client
        response_bytes = response_to_byte(response)
        with open(f'server_comm_log.txt', 'a') as f:
            f.write(f"server SENDING an MPITaskResponse of size {len(response_bytes)} bytes \n")

        if response.status == MPIServerStatus.ERROR.value:
            raise Exception("Server returned an error, stopping the client.")

        # TODO desrialize later before decryption
        # model = byte_to_model(response.payload)
