...
class ClientAgent:
    ...
    def get_parameters(
        ...
        # Encrypt and compress parameters after training using CKKS
        encrypted_params = {}
        ...
        # Include CKKS context in metadata
        metadata['ckks_context'] = self.ckks_context.serialize()

        self.logger.info(f">>> AT client {self.client_id}, done with get parameters")
    ...
    # TODO :D load, deserialize, decrypt parameters, and save reencrypted global model
    def load_parameters(self, encrypted_params) -> None:
        ...
        self.logger.info(f"client id: {self.client_id} starting decrypting and reencrypting")
        decrypted_params = {}
                ...
                with lock:  # Acquire the lock (blocks indefinitely)
                    for name, encrypted_param_serialized in encrypted_params.items():
                        if isinstance(encrypted_param_serialized, bytes):
                            ...
                            reencrypted_param = ts.ckks_vector(self.ckks_context, decrypted_param)
                            encrypted_params[name] = reencrypted_param.serialize()
                        elif isinstance(encrypted_param_serialized, torch.Tensor):
                            decrypted_params = copy.deepcopy(encrypted_params)
                            break
                    self.logger.info(f"client id: {self.client_id} saving reencrypted global model to file")
                    with open(model_file, 'wb') as f:
                        pickle.dump(encrypted_params, f)
        ...
        for name, decrypted_param in decrypted_params.items():
